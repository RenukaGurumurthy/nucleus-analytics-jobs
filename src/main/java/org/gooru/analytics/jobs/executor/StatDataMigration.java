...
public class StatDataMigration implements JobInitializer{
	...
	private static ResultSet getPublisherQueue(String metricsName) {
		...
		try {
			Statement select = QueryBuilder.select().all()
					.from(eventCassandraClusterClient.getCassKeyspace(), Constants.STAT_PUBLISHER_QUEUE)
	...
	private static ColumnList<String> getStatMetrics(String gooruOid) {
		...
		try {
			result = archivedCassandraClusterClient.getCassandraKeyspace()
					.prepareQuery(archivedCassandraClusterClient.accessColumnFamily("live_dashboard"))
	...
	private static boolean balanceCounterData(String clusteringKey, String metricsName, Long metricsValue) {
		try {
			...
			ResultSetFuture resultFuture = analyticsUsageCassandraClusterClient.getCassandraSession()
	...
	private static void indexingES(String indexName, String indexType, String id, XContentBuilder contentBuilder) {
		elasticsearchClusterClient.getElsClient().prepareIndex(indexName, indexType, id).setSource(contentBuilder)
				.execute().actionGet();
	}
}
