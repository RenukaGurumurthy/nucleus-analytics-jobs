...
public class ProcessCSVFile {
	...
	public static void main(String args[]) {
		...
		processCsv(questionsCsv, "question", "migrate");*/
		String standardsCsv = "/tmp/standards.csv";
		processCsv(standardsCsv, "standards", null);
	...
	private static void insertIntoQueue(String gooruOid, String type) {
		try {
			Insert insertStmt = QueryBuilder.insertInto("event_logger_insights", "stat_publisher_queue")
					.value("metrics_name", "migrateMetrics").value("gooru_oid", gooruOid)
					.value("event_time", System.currentTimeMillis()).value("type", type);
			ResultSetFuture resultSetFuture = eventCassandraClusterClient.getCassandraSession()
					.executeAsync(insertStmt);
			resultSetFuture.get();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void insertIntoTaxonomyParentNode(String key) {
		try {
			Insert insertStmt = QueryBuilder.insertInto("insights_qa", "taxonomy_parent_node").value("row_key", key)
					.value("subject_id", "NA").value("course_id", "NA").value("domain_id", "NA")
					.value("standards_id", key).value("learning_targets_id", "NA").value("item_count", 1);
			ResultSetFuture resultSetFuture = eventCassandraClusterClient.getCassandraSession()
					.executeAsync(insertStmt);
			resultSetFuture.get();
		} catch (Exception e) {
