...
public class ProcessItemUpdate {
  ...
  public void execute() {
    ...
    TransactionExecutor.executeWithAnalyticsDBTransaction(new DBHandler() {
      ...
      public Object execute() {
        ...
        String displayCode = data.isNull(AttributeConstants.DISPLAY_CODE) ? null : data.getString(AttributeConstants.DISPLAY_CODE);
        if (contentGooruId != null && contentFormat != null && AttributeConstants.CONTENT_FORMAT_FOR_TITLES.matcher(contentFormat).matches()) {
          updateContentTable(contentGooruId, title, subject,displayCode);
        }
        return null;
      }
    });
  ...
  private void updateContentTable(String contentGooruId, String title, String taxSubjectId, String displayCode) {
    if (title != null) {
      LOGGER.debug("contentGooruId : {} - title : {} ", contentGooruId, title);
      Base.exec(QueryConstants.UPDATE_CONTENT, title, taxSubjectId,displayCode, contentGooruId);
