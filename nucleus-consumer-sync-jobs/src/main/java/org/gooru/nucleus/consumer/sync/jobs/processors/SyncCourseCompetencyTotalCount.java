...
public class SyncCourseCompetencyTotalCount extends TimerTask {
  private static final Logger LOGGER = LoggerFactory.getLogger(SyncCourseCompetencyTotalCount.class);
  ...
  private SyncCourseCompetencyTotalCount() {
  }
  ...
  @Override
  public void run() {
    LOGGER.debug("SyncCourseCompetencyTotalCount is running..");
    ...
    if (this.config.isNull(AttributeConstants.EXPORT_QUERY)) {
      ...
      if (!this.config.isNull(AttributeConstants.IMPORT_TABLE)) {
        ...
        TransactionExecutor.executeWithAnalyticsDBCopyMgr(this.config.getString(AttributeConstants.IMPORT_TABLE), attributes, dumpLocation,
                dumpFileName);
        LOGGER.info("SyncCourseCompetencyTotalCount Imported..");
      }
